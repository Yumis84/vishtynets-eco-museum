...
const init=()=>{
  ...
  const activate=t=>{if(!t)return;if(t.kind==='intro'){document.querySelector('.audio-guide-start')?.click();return}const card=document.querySelector(`.audio-guide-track-card-v2[data-track-id="${CSS.escape(t.id)}"]`);if(!card)return;card.click();setTimeout(()=>document.querySelector('.audio-guide-player audio')?.play().catch(()=>{}),150)};
  ...
  const s=document.createElement('style');s.dataset.audioguidePlayerNav='2';s.textContent='.audio-guide-player-nav{grid-column:1 / -1;width:100%;box-sizing:border-box;display:flex;gap:12px;justify-content:center;align-items:center;margin:10px 0 0;padding:0}.audio-guide-player-nav button{box-sizing:border-box;width:44px;height:44px;min-width:44px;max-width:44px;padding:0!important;margin:0!important;border:1px solid #c8cec4;border-radius:50%;background:#f7f6f0;color:#344638;font-size:27px;font-weight:500;line-height:44px;text-align:center;display:flex;align-items:center;justify-content:center;cursor:pointer;box-shadow:0 2px 6px rgba(0,0,0,.08)}.audio-guide-player-nav button:active{transform:scale(.96)}.audio-guide-player-nav button:disabled{opacity:.35;cursor:default}.audio-guide-transcript-note{font-size:16px!important;line-height:1.65!important;margin-top:16px!important}.audio-guide-transcript-note p{margin:0 0 12px}';document.head.appendChild(s)
};
