...
const init=()=>{
  ...
  const activate=t=>{if(!t)return;if(t.kind==='intro'){document.querySelector('.audio-guide-start')?.click();return}const card=document.querySelector(`.audio-guide-track-card-v2[data-track-id="${CSS.escape(t.id)}"]`);if(!card)return;card.click();setTimeout(()=>{player.scrollIntoView({behavior:'smooth',block:'center'});const audio=player.querySelector('audio');audio?.play().catch(()=>{});},120)};
  ...
  const sync=()=>{const current=getCurrent(),i=Math.max(0,ordered.findIndex(t=>t.id===current?.id));prev.disabled=i<=0;next.disabled=i<0||i>=ordered.length-1};
