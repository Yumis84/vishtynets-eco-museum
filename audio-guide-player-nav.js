...
const init=()=>{
 ...
 const activate=t=>{if(!t)return;const card=document.querySelector(`.audio-guide-track-card-v2[data-track-id="${CSS.escape(t.id)}"]`);if(!card)return;card.click();setTimeout(()=>player.querySelector('audio')?.play().catch(()=>{}),120)};
 ...
 prev.onclick=e=>{e.preventDefault();e.stopPropagation();const i=ordered.findIndex(t=>t.id===getCurrent()?.id);if(i>0)activate(ordered[i-1])};
 next.onclick=e=>{e.preventDefault();e.stopPropagation();const i=ordered.findIndex(t=>t.id===getCurrent()?.id);if(i>=0&&i<ordered.length-1)activate(ordered[i+1])};
