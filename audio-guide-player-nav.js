...
const init=()=>{
 ...
 const getCurrent=()=>{const id=document.querySelector('.audio-guide-track-card-v2.is-selected')?.dataset.trackId;return ordered.find(t=>t.id===id)||ordered.find(t=>t.title===player.querySelector('strong')?.textContent?.trim())||ordered[0]};
 const activate=t=>{if(!t)return;const card=document.querySelector(`.audio-guide-track-card-v2[data-track-id="${CSS.escape(t.id)}"]`);if(!card)return;card.click();setTimeout(()=>player.querySelector('audio')?.play().catch(()=>{}),180)};
