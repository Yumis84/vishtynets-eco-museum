...
function injectVoice(){
 ...
 const player=document.querySelector('.screen-audio .audio-guide-player');
 const anchor=track||player;
 if(!anchor)return false;
 const parent=anchor.parentElement;
 if(!parent)return false;
 document.querySelectorAll('.audio-guide-voice-selector').forEach(el=>{if(!parent.contains(el))el.remove()});
 let existing=parent.querySelector('.audio-guide-voice-selector');
 if(existing){if(existing.previousElementSibling!==anchor)anchor.insertAdjacentElement('afterend',existing);const select=existing.querySelector('[data-audio-voice-select]');if(select)select.value=selectedId;return true}
 anchor.insertAdjacentElement('afterend',buildSelector());
 return true;
}
