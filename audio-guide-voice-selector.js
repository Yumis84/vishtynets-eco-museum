...
function injectVoice(){
 const hero=document.querySelector('.screen-audio .audio-guide-hero');
 ...
 if(existing){if(existing.previousElementSibling!==hero)hero.insertAdjacentElement('afterend',existing);const select=existing.querySelector('[data-audio-voice-select]');if(select)select.value=selectedId;return true}
 hero.insertAdjacentElement('afterend',buildSelector());
 return true;
}
