(function(){
  'use strict';
  const root=document.querySelector('[data-reviews-wall]'); if(!root)return;
  const card=root.querySelector('[data-review-card]');
  const canvas=root.querySelector('[data-review-canvas]');
  const ctx=canvas&&canvas.getContext('2d'); let drawing=false,last=null,color='#53684d',size=4;
  function fit(){if(!canvas||!card)return;const r=card.getBoundingClientRect(),d=window.devicePixelRatio||1;canvas.width=Math.round(r.width*d);canvas.height=Math.round(r.height*d);canvas.style.width=r.width+'px';canvas.style.height=r.height+'px';ctx.setTransform(d,0,0,d,0,0)}
  function point(e){const r=canvas.getBoundingClientRect();return{x:e.clientX-r.left,y:e.clientY-r.top}}
  if(canvas){canvas.addEventListener('pointerdown',e=>{drawing=true;canvas.setPointerCapture(e.pointerId);last=point(e)});canvas.addEventListener('pointermove',e=>{if(!drawing)return;const p=point(e);ctx.strokeStyle=color;ctx.lineWidth=size;ctx.lineCap='round';ctx.lineJoin='round';ctx.beginPath();ctx.moveTo(last.x,last.y);ctx.lineTo(p.x,p.y);ctx.stroke();last=p});['pointerup','pointercancel','pointerleave'].forEach(x=>canvas.addEventListener(x,()=>{drawing=false;last=null}));}
  root.querySelectorAll('[data-color]').forEach(b=>b.addEventListener('click',()=>color=b.dataset.color));
  root.querySelectorAll('[data-size]').forEach(b=>b.addEventListener('click',()=>size=Number(b.dataset.size)));
  root.querySelector('[data-clear]')?.addEventListener('click',()=>ctx&&ctx.clearRect(0,0,canvas.width,canvas.height));
