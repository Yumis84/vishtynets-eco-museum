(function(){
  ...
  const ctx=canvas&&canvas.getContext('2d'); if(!card||!canvas||!ctx)return;
  let drawing=false,last=null,color='#53684d',size=4;
  function fit(){const r=card.getBoundingClientRect(),d=window.devicePixelRatio||1;if(!r.width||!r.height)return;canvas.width=Math.round(r.width*d);canvas.height=Math.round(r.height*d);canvas.style.width=r.width+'px';canvas.style.height=r.height+'px';ctx.setTransform(d,0,0,d,0,0)}
  ...
  const observer=new MutationObserver(fit); observer.observe(root,{attributes:true,subtree:true,attributeFilter:['class','style']});
})();