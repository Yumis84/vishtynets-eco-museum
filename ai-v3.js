...
function init(){
 ...
 if(screen&&!screen.querySelector('.ai-header')){const header=document.createElement('header');header.className='ai-header';header.innerHTML='<div class="ai-header-icon" aria-hidden="true">✦</div><div class="ai-header-copy"><span class="eyebrow">Музейный помощник</span><h1>AI-консультант</h1></div>';const copy=header.querySelector('.ai-header-copy');if(heading){heading.textContent='Спросите о Роминтской пуще';copy.append(heading)}if(intro){intro.textContent='Помощник поможет найти места, статьи и маршруты.';copy.append(intro)}screen.insertBefore(header,card)}
 card.querySelector('.ai-orb')?.remove();
 const oldMessages=card.querySelector('.ai-messages');if(oldMessages)oldMessages.remove();
 const oldPrompts=card.querySelector('.ai-prompts');if(oldPrompts)oldPrompts.remove();
 const oldInput=card.querySelector('.ai-input');if(oldInput)oldInput.remove();
