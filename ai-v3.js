...
function init(){
 ...
 card.dataset.aiInitialized='1';card.hidden=false;card.style.display='flex';
 ...
 if(screen&&!screen.querySelector('.ai-header')){const header=document.createElement('header');header.className='ai-header';header.innerHTML='<button class="ai-close" data-nav="home" type="button" aria-label="Закрыть AI-консультант">×</button><div class="ai-header-icon" aria-hidden="true">✦</div><div class="ai-header-copy"><span class="eyebrow">Музейный помощник</span><h1>AI-консультант</h1></div>';const icon=header.querySelector('.ai-header-icon');icon.style.cssText='width:22px;height:22px;flex:0 0 22px;border-radius:50%;background:#34523d;color:#fff;display:grid;place-items:center;font-size:11px;line-height:1;margin-left:-2px';const copy=header.querySelector('.ai-header-copy');if(heading){heading.textContent='Спросите о Роминтской пуще';copy.append(heading)}if(intro){intro.textContent='Помощник поможет найти места, статьи и маршруты.';copy.append(intro)}screen.insertBefore(header,card)}
 card.querySelector('.ai-orb')?.remove();
 ...
 form.addEventListener('submit',e=>{e.preventDefault();const message=input.value;input.value='';ask(message)});prompts.querySelectorAll('button').forEach(b=>b.addEventListener('click',()=>ask(b.textContent)));
}
